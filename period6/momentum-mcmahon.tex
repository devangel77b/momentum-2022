...
\author{Alexander Hamilton McMahon}
\email{amcmahon\_25@mbs.net}
...
\section{Discussion}
\student{Even though we lose some energy, most energy is conserved, so we stated that energy is conserved. The amount of energy lost was \qty{-0.01\pm0.02}{\joule} and we were left with 80\% of energy. We had multiple people run the experiment three times so that we could minimize the room for error.}

\student{We tried to minimize the amount of error by running the test multiple times but certain variables can cause error in the experiment. Some errors that we can experience are sensor or online errors, people doing the experiment wrong, or any other situation. We ran the 80 times, but we could run it more times to see if it would have the same or different results.}

\section{Acknowledgements}
We thank \student{R.~Magnotta, K.~Patino, A.~Upadya, J.~Shwartz,} C.~Payette, J.~Bartholomew, B.~Turner, K.~Mauger, and S.~McCormick for useful advice and comments on the manuscript, and A.~Hahn, S.~Kealy, K~Muttick, and A.~Sankar, for advice on the design of experiment and learning objectives. Major lab equipment was gifted by the Alumni Association at Morristown-Beard School. This work was funded by ONR \#N123456789, AFOSR, NIH, and the National Science Foundation. 
