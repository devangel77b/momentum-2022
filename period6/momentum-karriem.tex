\documentclass[aps,prl,wreprint]{revtex4-1}
...
\begin{document}
%\linenumbers
\title{Is kinetic energy conserved?} 
\author{Raidah Karriem}
\email{rkarriem\_25@mbs.net}
...
\begin{abstract}
\student{Kinetic energy is a quantitative property that is transferred from an object, body, or physical system to another by motion. The form of kinetic energy is a conserved quality that is converted but neither created or destroyed. Kinetic energy is the product of one half mass times velocity squared $KE=\frac{1}{2}mv^2$. We examined if energy is conserved during an elastic collision using an instrumented, one-dimensional cart system. There was a track, along with sonar sensors with two carts placed on top. Cart A was pushed to cart B with hopes to collect the amount of energy changed. With the collection of data over four different classes, we found that energy is conserved.}
\end{abstract}
\maketitle
...
\section{Discussion}
\student{Our results in \fref{fig:results2} suggest kinetic energy is conserved during elastic collisions. For kinetic energy to be conserved, the change in energy $\Delta E$ must be zero ($H_0$); alternatively, energy could have been gained ($H_1$) or lost ($H_2$).  As shown in \fref{fig:results2}, $\Delta E$ was equal to \qty{-0.01 \pm0.02}{\joule}, which was not significantly different from zero. Therefore, we reject our alternative hypotheses and provisionally accept that energy is conserved.}

\student{However, when we normalized the energy loss by the initial energy we found that around 20\% energy was lost in the collision. This is likely due to the collision being nonideal.}

\subsection{Experimental limitations} 
\student{We observed an experimental error of \qty{0.02}{\joule}, which affects how precisely we can claim energy was conserved. During the collisions, the weight each varied because the amount of force in pushing the cart was not the same every time. Variation in applied force can affect the data. Another limitation is the small size of our test apparatus. If the collision were between two very large objects (e.g. two busses), we may have observed different findings, considering our wide error bands, that limit how far we can extrapolate our claims.}
