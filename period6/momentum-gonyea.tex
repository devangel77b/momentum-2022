...
\section{Discussion}
\subsection{Is energy conserved during elastic collisions?}
\student{A perfectly elastic collision conserves energy, however, in practice it is difficult to achieve a perfectly elastic collision. As shown in \fref{fig:results2}, there is a small energy loss, \qty{-0.01\pm0.02}{\joule}, which is not significantly different from zero owing to the large experimental error. When the first car hits the second, the second car takes the energy, minus a small loss. On each further bounce, the system loses more energy each time; in our tests the energy loss was approximately 20\% of the initial energy of the system.}

\subsection{Sources of experimental error}
\student{The large experimental error makes it difficult to conclusively resolve whether is energy is conserved during real, achievable elastic collisions. We may have needed different track lengths, and perhaps more diverse weights and separate vehicles of different sizes. We also could have been throwing at similar speeds, so we would have a constant and can have a more diverse table. Drag and friction may also have been a problem, since they would act as ways to slow down the cars.}
