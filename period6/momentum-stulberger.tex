...
\author{Rebecca Stulberger}
\email{rstulberger\_25@mbs.net}
...
\begin{abstract}
\student{Kinetic energy is the energy an object has when moving, and is the product of $\frac{1}{2}$ mass times velocity ($KE=\frac{1}{2}mv^2$). It is potentially useful for understanding the mechanics of collisions. We examined whether kinetic energy is conserved in an elastic collision. We used two carts on an instrumented one-dimensional test track to measure kinetic energy of the system before and after an elastic collision. We found that kinetic energy was conserved.} 
%Momentum is the product of mass times velocity ($\vec{p}=m\vec{v}$), and kinetic energy of a moving mass is given by $\frac{1}{2}mv^2$. Both are potentially useful for understanding the mechanics of collisions, but while previous studies have examined momentum conservation during inelastic collisions \cite{ortega-2021-momentum}, they have not considered energy conservation during elastic collisions. \student{We examined whether kinetic energy is conserved in an elastic collision. We used two carts and one track to test if kinetic energy is conserved. We found that kinetic energy is conserved in an elastic collision.}
\end{abstract}
\maketitle
...
\section{Discussion}
\student{As shown in \fref{fig:results2}, our results support the hypothesis that kinetic energy is conserved in an elastic collision. Between periods 4, 6, and 8, 80 collisions were completed. Once two or three collisions were completed, someone else would switch in and create their own collisions. Weights were also changed when people changed. As in, weights were taken away and added, but it was not every time that people changed. The weights ranged between \qtyrange{0.3028}{0.6765}{\kilo\gram}. The data showed that the kinetic energy conserved was not significantly different from zero (\qty{-0.01\pm0.02}{\joule}). As a result, we rule out the alternative hypotheses, that kinetic energy might increase ($H_1$) or decrease ($H_2$).}

\student{There were multiple limitations in our experiment. For one, we were limited in collision size. We were limited by how long the track was, so the collision could only be to a certain size. Meaning that if the track was longer there might be a different out come such as the energy conserved reaching even closer to zero, The experimental error is shown through different weights, force, and force on cart A. However, doing the experiments on different days would combat the errors. Not only will different weights affect the experiment but also using the scale to measure the weights overall. The sensors and the app that does the calculation also may have affected the outcome. The sensors and apps could glitch or timing could be off. This would result in the velocity being off, whether it raises or lowers. In the end, it affects how detectible a change in energy would be.}


\section{Acknowledgements}
We thank \student{C.~Sussman, K.~Patino,} C.~Payette, J.~Bartholomew, B.~Turner, K.~Mauger, and S.~McCormick for useful advice and comments on the manuscript, and A.~Hahn, S.~Kealy, K~Muttick, and A.~Sankar, for advice on the design of experiment and learning objectives. Major lab equipment was gifted by the Alumni Association at Morristown Beard School. This work was funded by ONR \#N123456789, AFOSR, NIH, and the National Science Foundation. 
