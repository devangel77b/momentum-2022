...
\begin{document}
\linenumbers
\title{Consevation of energy}
\author{Kevin Chen}
\email{kchen\_25@mbs.net}
...
\begin{abstract}
\student{The Law of Energy states that energy cannot be created nor destroyed; it can only change forms. Kinetic energy can be calculated with the formula $KE=\frac{1}{2}mv^2$. We examined whether energy was conserved by colliding two carts together on a rail system. We found that energy was conserved.}
\end{abstract}
\maketitle
...
\section{Discussion}
\student{The results of our experiments show that energy is conserved. The average change in energy that we found in our experiments was \qty{\pm0.01}{\joule}, which is close to 0. Another way we minimized experimental error was by having different people initiate the test, as well as changing weights and force.}

\subsection{Experimental error}
\student{Though we did our best to minimize experimental error, there is still a possibility of error in our experiments. We made sure to run the experiments many times in order to minimize experimental error. There could have been errors with our sensors or online system. There may also have been experimental errors with friction, different atmospheric forces, and air resistance. With our limited resources, we were also not able to experiment if our conclusions would apply to larger scaled experiments.}
