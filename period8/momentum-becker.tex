...
\linenumbers
\title{Is 30\% a lot?}
\author{Adison Becker}
\email{abecker\_25@mbs.net}
...
\begin{abstract}
Momentum is the product of mass times velocity ($\vec{p}=m\vec{v}$), and kinetic energy of a moving mass is given by $\frac{1}{2}mv^2$. Both are potentially useful for understanding the mechanics of collisions, but while previous studies have examined momentum conservation during inelastic collisions \cite{ortega-2021-momentum}, they have not considered energy conservation during elastic collisions. We examined \student{the energy conserved in an elastic collision between carts on a track}. \student{We examined 1D inelastic collisions between two carts at varying weights to see if energy decreases. Many different people conducted the same experiment, again all different weights and directions. We found that energy is not conserved, it decreases.
}\end{abstract}
\maketitle
...
\section{Discussion}
\student{We conducted 81 different collisions throughout the experiment. The start of the collision had $m_1$ moving with $m_2$ stopped. This scenario is $E_0 = \frac{1}{2}mv_{1,0}^2$, or original energy. The end of the collision is when $m_1$ hits $m_2$. $m_1$ should be slowed and or stopped and $m_2$ should now be moving. This scenario is $E_f=\frac{1}{2} m_1 v_{1,f}^2 + \frac{1}{2} m_2 v_{2,f}^2$, or energy final. If these two equations were equivalent, then that would mean energy was conserved. In our case $E_f$ was less than $E_0$, meaning energy was lost. The original energy average was \qty{0.038}{\joule}. The change in energy was \qty{-0.01\pm0.02}{\joule}. This is not greatly different from zero, however almost 30\% of energy was lost.}

\student{How much can we say was experimental error and what caused the energy loss? We made our experiment almost personal error proof. We used many different people with their carts at various weights, this along with pushing the carts both ways takes gravity out of the cause. The bias would have been canceled out by the amount of times it was tested. The friction was very minimal, science can never have a frictionless surface. The same case with a perfect vacuum. The only feasible cause for energy loss would be during the collision. Our lab has its limits on what can be done. An experiment needs to be fairly small scale, in a less controlled environment i.e. weather conditions and kids, etc. The best experiment to do next would be one on a larger scale to see if the number is as close to zero. A larger scale with a more precise method could give us the correct solution. If E0 and Ef were larger numbers, we might see a larger difference in energy loss, or we could see a small number again. This would better answer the question if energy is conserved.}

\section{Acknowledgements}
We thank \student{FILLIN}, C.~Payette, J.~Bartholomew, B.~Turner, K.~Mauger, and S.~McCormick for useful advice and comments on the manuscript, and A.~Hahn, S.~Kealy, K~Muttick, and A.~Sankar, for advice on the design of experiment and learning objectives. Major lab equipment was gifted by the Alumni Association at Morristown-Beard School. This work was funded by ONR \#N123456789, AFOSR, NIH, and the National Science Foundation. 
