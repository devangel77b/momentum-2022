...
\begin{document}
\linenumbers
\title{Emma Teitelbaum has no title}
\author{Emma Teitelbaum}
\email{eteitelbaum\_25@mbs.net}
...
\begin{abstract}
Momentum is the product of mass times velocity ($\vec{p}=m\vec{v}$), and kinetic energy of a moving mass is given by $\frac{1}{2}mv^2$. Both are potentially useful for understanding the mechanics of collisions, but while previous studies have examined momentum conservation during inelastic collisions \cite{ortega-2021-momentum}, they have not considered energy conservation during elastic collisions. \student{We examined whether energy was conserved in an elastic collision using an instrumented test track. We found that the change in energy was a small amount (\qty{-0.01\pm0.02}{\joule}), not an amount significantly different from zero. On the other hand, the small amount was a significant percentage of the total energy.}
\end{abstract}
\maketitle
...
\section{Discussion}
\student{Based on our evidence, we found that the change in energy wasn’t significantly different from zero, which shows that energy was conserved. We can easily see that energy wasn’t gained, so at first glance we can see that energy was conserved. However, if we consider the fraction of the initial energy that was lost (20\%), we realize that energy might actually be lost instead of conserved.}

\student{To find out where the 20\% of the energy went, we can conduct an experiment. One experiment that you could do is instead of just one collision, you try two collisions, and then three collisions, and then four, and so on. When you test them out, you should get -20\% energy for each collision, which proves the point.}
